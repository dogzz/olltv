...
def _channel_packs(categories, content):
    """
    Create channel packs (subscriptions) listing
    """
    for category in categories:
        yield {
            'label': u'{0} [COLOR=gray][{1}][/COLOR]'.format(category.name, category.items),
            'url': plugin.get_url(action='channels_list', path=category.path, page='0'),
            'thumb': os.path.join(icons, content + '.png')
        }
...
@plugin.cached(180)
def _get_channel_packs(path):
    return exua.get_channel_packs(path)
...
@plugin.action('channel_packs')
def channel_packs(params):
    """
    Show channel packs (subscriptions) categories
    """
    if plugin.cache_pages:
        categories = _get_channel_packs('/account/subs')
    else:
        categories = exua.get_channel_packs('/account/subs')
    plugin.log('Channel packs: {0}'.format(categories))
    return _channel_packs(categories, params['content'])
...
def root(params):
    ...
    if plugin.authorization:
        bookmarks_item = {'url': plugin.get_url(action='bookmarks'),
                          'thumb': os.path.join(icons, 'key.png')}
        if webclient.is_logged_in():
            bookmarks_item['label'] = '[{0}]'.format(_('Oll.tv login (logged in)'))
        else:
            bookmarks_item['label'] = '[{0}]'.format(_('Oll.tv login'))
...
@plugin.cached(30)
def _get_channel_list(path):
    return exua.get_channel_list(path)
...
def channels_list(params):
    ...
    page = int(params['page'])
    if plugin.cache_pages:
        channel_listing = _get_channel_list(params['path'])
    else:
        channel_listing = exua.get_channel_list(params['path'])
    plugin.log('Channel list: {0}'.format(channel_listing))
    return plugin.create_listing(_channel_list(params['path'], channel_listing), update_listing=False)


def _channel_list(path, channel_listing, page=0, is_search_result=False):
    """
    Create the list of channels
    """
    for item in channel_listing:
        yield {
            'label': item.title,
            'url': plugin.get_url(action='play_channel', path=item.path, referer=item.referer, label=item.title),
            'thumb': item.thumb
        }
...
def play_channel(params):
    ...
    label = params['label']
    stream = exua.get_channel_strem(path, referer)
    ...
    plugin.log('Playing path: {0}'.format(stream))
    player = exua.VideoPlayer(popstack=True)
    listitem = xbmcgui.ListItem(label)
    listitem.setInfo('video', {'Title': '%s, %s)' % (label, label.decode('utf-8'))})
    player.play_item(stream, listitem)
    player.WaitForVideoEnd()
    # return CHANNEL_LIST
    # return stream
